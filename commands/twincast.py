...
class Twincast:  # Inside this class we make our own command.
                        ...
                        if re.match(self.current_round['pattern1'], word):
                            if re.match(self.current_round['pattern2'], word):
                                ...
                                if r.table('users').get(user_id).run(self.conn):
                                    ...
                                    r.table('users').insert({"id": user_id, "single_casts": 1}).run(self.conn)
                                    print("added " + user_id + " to the db")
                                print(r.table('users').get(user_id).run(self.conn))
                            ...
                            if r.table('users').get(user_id).run(self.conn):
                                ...
                                r.table('users').insert({"id": user_id, "failures": 1}).run(self.conn)
                                print("added " + user_id + " to the db")
                            print(r.table('users').get(user_id).run(self.conn))
    ...
    async def create_global_leaderboard(self):
        ...
        await self.bot.get_channel(232937599537381377).send(embed=Embed(
            title="Top Twincasters", description=str(r.table('users').order_by(r.desc('twincasts')).limit(3).
                                                     run(conn))))
    async def create_round_leaderboard(self,round_name: str):
    ...
    async def update_leaderboard(self):
        ...
        if await (self.bot.get_channel(232937599537381377).history()).next():
            ...
            await self.create_global_leaderboard()
